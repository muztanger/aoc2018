...
for guard in guard_sleep:
    sleep = sum(guard_sleep[guard])
    if sleep > max_sleep_time:
        max_sleep_time = sleep
        max_guard = guard
        best_time = max(enumerate(guard_sleep[guard]), key=itemgetter(1))[0]
print("{} * {} = {}".format(max_guard, best_time, max_guard * best_time))
